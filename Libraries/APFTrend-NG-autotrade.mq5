...
int getSignal(const int type, const int idx, const int i, const datetime &time[],
        ...
        const double &MAFast[], const double &MASlow[], const double &MA25[], const double &MA50[], const double &MA100[], const double &MA200[], stru_filter &filter, stru_trade &apftrade, stru_counters &mycounters, stru_stats &pricestats) export
  {
     ...
     bool VWAP_CROSS = (high[i-1]>VWAP[i-1] && low[i-1]<VWAP[i-1]);
     bool VWAP_LASTCROSS = (high[i-2]>VWAP[i-2] && low[i-2]<VWAP[i-2]);
     ...
     if (pricestats.bars_day>5 && date_candle.hour<16 && aux_abs>aux_abslast) {
        ...
        if (type==1 && pricestats.bars_day>8 && aux_abs>aux_abslast && aux_abs>candle_hi/2 && aux_abs>candle_lo/2) {
           if (mycounters.c_up<3 && body_up>0 && CCI[i-3]<-210 && CCI[i-1]>-110 && close[i-1]>HILO2[i-1]) aux_dir+=15;
           else if (mycounters.c_down<3 && body_down>0 && CCI[i-3]>210 && CCI[i-1]<110 && close[i-1]<HILO2[i-1] && high[i-3]>upBand[i-3]) aux_dir-=15;
           if (date_candle.hour>12) {
              if (mycounters.c_up<3 && body_up>0 && body_up>2*candle_hi && close[i-1]>high[i-2] && (CCI[i-2]<-210 || CCI[i-3]<-210) && close[i-3]<loBand[i-2] && open[i-1]<VWAP[i-1]) aux_dir+=15;
              else if (mycounters.c_down<3 && body_down>0 && body_down>2*candle_lo && close[i-1]<low[i-2] && (CCI[i-2]>200 || CCI[i-3]>200) && close[i-3]>upBand[i-2] && open[i-1]>VWAP[i-1]) aux_dir-=15;
           }
        }
     }
     ...
     if (date_candle.hour==14 && date_candle.min>25) {
        ...
        if (type==1 && filter.NO_VWAPCROSS && (tick_volume[i-1]>tick_volume[i-2] || tick_volume[i-1]>tick_volume[i-3])) {
           if (mycounters.c_up==2 && body_up>0 && RSI[i-3]<25 && CCI[i-3]<-135 && CCI[i-1]>-70 && high[i-1]<VWAP[i-1] && (high[i-1]>HILO[i-1] || high[i-1]>HILO2[i-1]) && close[i-1]-low[i-1]>2*aux_abslast && close[i-1]>high[i-2] && close[i-1]>(open[i-3]+close[i-3])/2 && close[i-3]<loBand2[i-3] && close[i-1]>loBand2[i-1]) aux_dir+=20;
     ...
     if (CCI[i-1]>50 && (type==0 || (type==1 && VWAP[i-1]>0 && date_candle.hour<17))) {
        ...
        if (filter.HILO_BUY) {
           ...
           if (type==1 && date_candle.hour>9 && filter.VWAP_CROSS && mycounters.hilobuy<5 && CCI[i-1]>-30 && CCI[i-1]<100 && body_up>5*candle_hi && low[i-1]-HILO[i-1]<candle_lo/3 && open[i-1]<MAFast[i-1] && close[i-1]>MAFast[i-1]) aux_dir+=5;
        ...
        if (filter.HILO_INVERTBUY) {
            aux_dir+=3;
            if (type==1) {
               if (mycounters.c_up>1 && filter.VWAP_DOWN && i-pricestats.vwap_idx>30 && high[i-1]<VWAP[i-1] && MA100[i-1]>VWAP[i-1] && MA25[i-1]<MA50[i-1] && MA50[i-1]<MA100[i-1] && high[i-1]<MA200[i-1]);
               else {
        ...
        if (aux_dir>1 && (type==1 || apftrade.trade_dir==1)) {
           ...
           if (type==1 && aux_dir>2) {
              ...
              if (aux_dir>2 && pricestats.bars_day<10 && filter.VWAP_CROSS && filter.VWAP_LASTCROSS && force_trade==0 && CCI[i-1]>100 && close[i-1]>upBand[i-1] && low[i-1]<MAFast[i-1] && high[i-1]>MAFast[i-1] && low[i-1]<MA25[i-1] && high[i-1]>MA25[i-1] && (date_candle.day_of_week==1 || date_candle.day_of_week==5)) aux_dir=0;
              ...
              if (force_trade==0) {
                 if (filter.VWAP_CROSS && aux_dir>2) {
                    ...
                    if (aux_dir>2 && date_candle.hour<11 && filter.VWAP_LASTCROSS && CCI[i-1]>110 && pricestats.max_dayprice-high[i-1]<aux_abs/5 && high[i-1]>MAFast[i-1] && low[i-1]<MAFast[i-1] && high[i-1]>MA25[i-1] && low[i-1]<MA25[i-1] && high[i-2]>MAFast[i-2] && low[i-2]<MAFast[i-2] && high[i-2]>MA25[i-2] && low[i-2]<MA25[i-2] && ((MA50[i-1]-high[i-1]<aux_abs/5 && close[i-1]<MA50[i-1]) || (MA100[i-1]-high[i-1]<aux_abs/5 && close[i-1]<MA100[i-1]))) aux_dir=0;
                 }
                 ...
                 if (aux_dir>2 && mycounters.c_up>2 && (date_candle.day_of_week==1 || date_candle.day_of_week==5) && filter.VWAP_DOWN && i-pricestats.vwap_idx>15 && CCI[i-1]>110 && candle_hi>0 && high[i-1]<VWAP[i-1] && (MA100[i-1]<MA100[i-3] || MA200[i-1]<MA200[i-3]) && close[i-1]<MA25[i-1] && MA25[i-1]-high[i-1]<aux_abs/2 && MA25[i-1]<MA50[i-1] && MA50[i-1]<MA100[i-1] && MA100[i-1]<MA200[i-1]) aux_dir=0;
              ...
              if (aux_dir>2 && aux_dir<12 && CCI[i-1]>130 && force_trade==0 && open[i-1]>VWAP[i-1] && MA100[i-1]>MA50[i-1] && MA50[i-1]>MA25[i-1] && close[i-1]>upBand[i-1]) aux_dir=0;
              if (aux_dir>2 && aux_dir<15 && date_candle.hour==9 && date_candle.min==50 && candle_hi>0 && MA100[i-1]>MA50[i-1] && MA50[i-1]>MA25[i-1]) aux_dir=0;
        ...
        if (filter.HILO_SELL) {
           ...
           if (type==1 && date_candle.hour>9 && filter.VWAP_CROSS && mycounters.hilosell<5 && CCI[i-1]<30 && CCI[i-1]>-100 && body_down>5*candle_lo && HILO[i-1]-high[i-1]<candle_hi/3 && open[i-1]>MAFast[i-1] && close[i-1]<MAFast[i-1]) aux_dir-=5;
        ...
        if (filter.HILO_INVERTSELL) {
           aux_dir-=3;
           if (type==1) {
              if (mycounters.c_down>1 && filter.VWAP_UP && i-pricestats.vwap_idx>30 && low[i-1]>VWAP[i-1] && MA100[i-1]>VWAP[i-1] && MA25[i-1]>MA50[i-1] && MA50[i-1]>MA100[i-1] && low[i-1]>MA200[i-1]);
              else {
        ...
        if (aux_dir<-1 && (type==1 || apftrade.trade_dir==2)) {
           ...
           if (type==1 && aux_dir<-2) {
              ...
              if (aux_dir<-2 && pricestats.bars_day<10 && filter.VWAP_CROSS && filter.VWAP_LASTCROSS && force_trade==0 && CCI[i-1]<-100 && close[i-1]<loBand[i-1] && high[i-1]>MAFast[i-1] && low[i-1]<MAFast[i-1] && high[i-1]>MA25[i-1] && low[i-1]<MA25[i-1] && (date_candle.day_of_week==1 || date_candle.day_of_week==5)) aux_dir=0;
              ...
              if (force_trade==0) {
                 if (aux_dir<-2 && filter.VWAP_CROSS) {
                    ...
                    if (aux_dir<-2 && date_candle.hour<11 && filter.VWAP_LASTCROSS && CCI[i-1]<-110 && low[i-1]-pricestats.min_dayprice<aux_abs/5 && high[i-1]>MAFast[i-1] && low[i-1]<MAFast[i-1] && high[i-1]>MA25[i-1] && low[i-1]<MA25[i-1] && high[i-2]>MAFast[i-2] && low[i-2]<MAFast[i-2] && high[i-2]>MA25[i-2] && low[i-2]<MA25[i-2] && ((low[i-1]-MA50[i-1]<aux_abs/5 && close[i-1]>MA50[i-1]) || (low[i-1]-MA100[i-1]<aux_abs/5 && close[i-1]>MA100[i-1]))) aux_dir=0;
                 }
                 ...
                 if (aux_dir<-2 && mycounters.c_down>2 && (date_candle.day_of_week==1 || date_candle.day_of_week==5) && filter.VWAP_UP && i-pricestats.vwap_idx>15 && CCI[i-1]<-110 && candle_lo>0 && MA50[i-1]>MA50[i-3] && (MA100[i-1]>MA100[i-3] || MA200[i-1]>MA200[i-3]) && close[i-1]>MA25[i-1] && low[i-1]-MA25[i-1]<aux_abs/2 && MA25[i-1]>MA50[i-1] && MA50[i-1]>MA100[i-1] && MA100[i-1]>MA200[i-1]) aux_dir=0;
              ...
              if (aux_dir<-2 && aux_dir>-12 && CCI[i-1]<-130 && force_trade==0 && RSI[i-1]<24 && open[i-1]<VWAP[i-1] && MA25[i-1]>MA50[i-1] && MA50[i-1]>MA100[i-1] && close[i-1]<loBand[i-1]) aux_dir=0;
              if (aux_dir<-2 && aux_dir>-15 && date_candle.hour==9 && date_candle.min==50 && candle_lo>0 && MA25[i-1]>MA50[i-1] && MA50[i-1]>MA100[i-1]) aux_dir=0;
     ...
     if (VOLUME_UP && filter.VWAP_CROSS) {
        ...
        else if (type==1 && filter.HILO_INVERTBUY && aux_dir>2 && aux_dir<12 && CCI[i-1]>210 && RSI[i-1]>70 && force_trade==0) aux_dir--;
        else if (type==1 && aux_dir>2 && aux_dir<12 && filter.VWAPCROSS && date_candle.hour<11 && CCI[i-1]>55 && mycounters.hilobuy>4 && high[i-1]<open[i-2]) aux_dir-=2;
        else if (type==1 && filter.HILO_INVERTBUY && date_candle.hour<11 && candle_hi>0 && CCI[i-1]>140 && close[i-1]>upBand[i-1] && pricestats.max_dayprice-high[i-1]<aux_abs/5 && close[i-1]>VWAP[i-1] && VWAP[i-1]>MA25[i-1] && MA25[i-1]>MA50[i-1] && MA50[i-1]>MA100[i-1]) aux_dir=0;
        ...
        else if (type==1 && filter.HILO_INVERTSELL && aux_dir<-2 && aux_dir>-12 && CCI[i-1]<-210 && force_trade==0) aux_dir++;
        else if (type==1 && aux_dir<-2 && aux_dir>-12 && filter.VWAPCROSS && date_candle.hour<11 && CCI[i-1]<-55 && mycounters.hilosell>4 && low[i-1]>open[i-2]) aux_dir+=2;
        else if (type==1 && filter.HILO_INVERTSELL && date_candle.hour<11 && candle_lo>0 && CCI[i-1]<-140 && close[i-1]<loBand[i-1] && low[i-1]-pricestats.min_dayprice<aux_abs/5 && close[i-1]<VWAP[i-1] && VWAP[i-1]<MA25[i-1] && MA50[i-1]>MA25[i-1] && MA100[i-1]>MA50[i-1]) aux_dir=0;
     ...
     if (filter.NO_VWAPCROSS) {
        ...
        if (type==1) {
           if (aux_dir>2) {
              if ((filter.HILO_SELL || filter.SAR_SELL) && filter.tdf_color==0 && i-pricestats.vwap_idx>30 && stdDev[i-1]<aux_abs/2 && high[i-1]<high[i-3] && date_candle.hour>13) aux_dir=0;
              if (aux_dir>2 && filter.NO_VWAPCROSS && (filter.VWAP_DOWN || VWAP[i-1]<VWAP[i-2]) && i-pricestats.vwap_idx>30 && CCI[i-1]>140 && high[i-1]<VWAP[i-1] && close[i-1]<MA50[i-1] && ((high[i-1]>MA50[i-1] && close[i-1]<MA50[i-1]) || (high[i-1]>MA100[i-1] && close[i-1]<MA100[i-1]))) aux_dir=0;
           }
           else if (aux_dir<-2) {
              if ((filter.HILO_BUY || filter.SAR_BUY) && filter.tdf_color==0 && i-pricestats.vwap_idx>30 && stdDev[i-1]<aux_abs/2 && low[i-1]>low[i-3] && date_candle.hour>13) aux_dir=0;
              if (aux_dir<-2 && filter.NO_VWAPCROSS && (filter.VWAP_UP || VWAP[i-1]>VWAP[i-2]) && i-pricestats.vwap_idx>30 && RSI[i-1]<32 && CCI[i-1]<-140 && low[i-1]>VWAP[i-1] && close[i-1]>MA50[i-1] && ((low[i-1]<MA50[i-1] && close[i-1]>MA50[i-1]) || (low[i-1]<MA100[i-1] && close[i-1]>MA100[i-1]))) aux_dir=0;
           }
     ...
     if (force_trade==0) {
        ...
        if (type==1) {
           if (aux_dir>2) {
              ...
              if (aux_dir>5 && aux_dir<14 && date_candle.hour==10 && force_trade==0 && open[i-1]<VWAP[i-1] && close[i-1]-VWAP[i-1]>2*(VWAP[i-1]-open[i-1]) && (close[i-1]>MA25[i-1] || close[i-1]>MA100[i-1]) && high[i-1]<MA50[i-1]) aux_dir=0;
              if (aux_dir>2 && aux_dir<14 && date_candle.hour<11 && CCI[i-1]>90 && candle_hi>0 && high[i-1]>upBand[i-1] && (low[i-1]<loBand[i-1] || low[i-2]<loBand[i-2]) && MA100[i-1]>MA50[i-1] && MA50[i-1]>MA25[i-1] && high[i-1]>MA100[i-1] && low[i-1]<MA25[i-1]) aux_dir=0;
              if (aux_dir>2 && aux_dir<14 && date_candle.hour>15 && CCI[i-1]>120 && MFI[i-1]>90 && RSI[i-1]>70 && low[i-1]>VWAP[i-1]) aux_dir-=2;
              if (aux_dir>2 && aux_dir<14 && filter.tdf_color==0 && mycounters.hilobuy>5 && CCI[i-1]>68 && CCI[i-1]-CCI[i-2]<1 && close[i-1]<high[i-2] && candle_hi>0 && low[i-1]>VWAP[i-1] && (tick_volume[i-1]<tick_volume[i-2] || tick_volume[i-1]<tick_volume[i-3])) aux_dir-=2;
              if (aux_dir>2 && aux_dir<14 && date_candle.hour>12 && filter.NO_VWAPCROSS && CCI[i-1]>90 && stdDev[i-1]<aux_abs/2 && i-pricestats.vwap_idx>15 && high[i-1]<VWAP[i-1] && MA200[i-1]>MA100[i-1] && MA100[i-1]-MA50[i-1]>aux_abs && MA50[i-1]-MA25[i-1]>aux_abs/2) aux_dir=0;
              if (aux_dir>2 && filter.VWAP_CROSS && filter.VWAP_LASTCROSS && mycounters.c_up==1 && stdDev[i-1]<aux_abs/2 && ((high[i-1]>upBand2[i-1] && high[i-2]>upBand2[i-2]) || (high[i-1]>upBand[i-1] && high[i-2]>upBand[i-2] && MA200[i-1]<MA200[i-2]))) aux_dir=0;
              if (aux_dir>2 && filter.VWAP_CROSS && filter.VWAP_LASTCROSS && MA200[i-1]>MA100[i-1] && MA100[i-1]>MA50[i-1] && MA50[i-1]>MA25[i-1] && high[i-1]>MA50[i-1] && close[i-1]<MA50[i-1] && high[i-1]>upBand[i-1] && MA200[i-1]<MA200[i-2] && MA100[i-1]<MA100[i-2]) aux_dir=0;
              if (aux_dir>2 && VWAP_CROSS && force_trade==0 && high[i-2]<VWAP[i-2] && CCI[i-1]>150 && close[i-1]>upBand[i-1] && low[i-2]<loBand2[i-2] && close[i-1]>MAFast[i-1]) aux_dir=0;
              if (force_trade==0 && filter.tdf_color==0) {  
                 if (aux_dir>2 && filter.VWAP_CROSS && pricestats.bars_day<6 && high[i-1]>MA200[i-1] && close[i-1]<MA200[i-1]) aux_dir=0;
                 if (aux_dir>2 && filter.VWAP_LASTCROSS && filter.HILO_INVERTBUY && CCI[i-1]>150 && candle_hi>0 && open[i-1]>MAFast[i-1] && high[i-3]<VWAP[i-3] && aux_abs<aux_abslast/2 && MathAbs(close[i-3]-open[i-3])<aux_abslast/2) aux_dir=0;
                 if (aux_dir>2 && filter.NO_VWAPCROSS && filter.VWAP_DOWN && i-pricestats.vwap_idx>30 && CCI[i-1]>120 && high[i-1]<VWAP[i-1] && close[i-1]<MA50[i-1] && ((high[i-1]>MA50[i-1] && close[i-1]<MA50[i-1]) || (high[i-1]>MA100[i-1] && close[i-1]<MA100[i-1]))) aux_dir-=2;
                 if (aux_dir>2 && filter.NO_VWAPCROSS && filter.VWAP_DOWN && filter.MIBAND_DOWN && CCI[i-1]>95 && candle_hi>0 && stdDev[i-1]<aux_abs/2 && i-pricestats.vwap_idx>20 && high[i-1]<VWAP[i-1] && MA50[i-1]<VWAP[i-1] && MA100[i-1]-MA50[i-1]>aux_abs && MA50[i-1]-MA25[i-1]>aux_abs/2 && MA100[i-1]<MA100[i-2]) aux_dir=0;
                 if (aux_dir>2 && filter.NO_VWAPCROSS && filter.HILO_INVERTBUY && date_candle.hour==13 && CCI[i-1]>100 && MFI[i-1]>71 && candle_hi>0 && high[i-1]<VWAP[i-1] && VWAP[i-1]-high[i-1]<body_up/2) aux_dir=0;
                 if (aux_dir>2 && filter.NO_VWAPCROSS && filter.HILO_INVERTBUY && date_candle.hour==13 && mycounters.c_up==1 && i-pricestats.vwap_idx>30 && CCI[i-1]>170 && candle_hi>0 && low[i-1]>VWAP[i-1]) aux_dir=0;
                 if (aux_dir>2 && filter.NO_VWAPCROSS && filter.VWAP_DOWN && filter.HILO_SELL && MA25[i-1]<VWAP[i-1] && close[i-1]<VWAP[i-1] && high[i-1]>HILO[i-1] && close[i-1]<HILO[i-1] && high[i-1]>MA25[i-1] && close[i-1]<MA25[i-1]) aux_dir--;
              }
           }
           else if (aux_dir<-2) {
              if (force_trade==0 && CCI[i-1]<-130 && close[i-1]<MA200[i-1] && MA200[i-1]<MA100[i-1]) aux_dir++;
              ...
              if (aux_dir<-2 && aux_dir>-10 && date_candle.hour<11 && CCI[i-1]<-90 && candle_lo>0 && low[i-1]<loBand[i-1] && (high[i-1]>upBand[i-1] || high[i-2]>upBand[i-2]) && MA100[i-1]<MA50[i-1] && MA50[i-1]<MA25[i-1] && high[i-1]>MA25[i-1] && low[i-1]<MA100[i-1]) aux_dir=0;
              if (aux_dir<-5 && aux_dir>-14 && date_candle.hour==10 && force_trade==0 && open[i-1]>VWAP[i-1] && VWAP[i-1]-close[i-1]>2*(open[i-1]-VWAP[i-1]) && (close[i-1]<MA25[i-1] || close[i-1]<MA100[i-1]) && low[i-1]>MA50[i-1]) aux_dir=0;
              if (aux_dir<-2 && aux_dir>-14 && date_candle.hour>15 && CCI[i-1]<-120 && MFI[i-1]<20 && RSI[i-1]<35 && high[i-1]<VWAP[i-1]) aux_dir+=2;
              if (aux_dir<-2 && aux_dir>-14 && filter.tdf_color==0 && mycounters.hilosell>5 && CCI[i-1]<-70 && CCI[i-2]-CCI[i-1]<1 && close[i-1]>low[i-2] && candle_lo>0 && high[i-1]<VWAP[i-1] && (tick_volume[i-1]<tick_volume[i-2] || tick_volume[i-1]<tick_volume[i-3])) aux_dir+=2;
              if (aux_dir<-2 && aux_dir>-14 && date_candle.hour>12 && filter.NO_VWAPCROSS && CCI[i-1]<-90 && stdDev[i-1]<aux_abs/2 && i-pricestats.vwap_idx>15 && high[i-1]<VWAP[i-1] && MA200[i-1]<MA100[i-1] && MA50[i-1]-MA100[i-1]>aux_abs && MA25[i-1]-MA50[i-1]>aux_abs/2) aux_dir=0;
              if (aux_dir<-2 && filter.VWAP_CROSS && filter.VWAP_LASTCROSS && mycounters.c_down==1 && stdDev[i-1]<aux_abs/2 && low[i-1]<loBand2[i-1] && low[i-2]<loBand2[i-2]) aux_dir=0;
              if (aux_dir<-2 && filter.VWAP_CROSS && filter.VWAP_LASTCROSS && aux_dir>-14 && MA25[i-1]>MA50[i-1] && MA50[i-1]>MA100[i-1] && MA100[i-1]>MA200[i-1] && low[i-1]<MA50[i-1] && close[i-1]>MA50[i-1] && low[i-1]<loBand[i-1] && MA200[i-1]>MA200[i-2] && MA100[i-1]>MA100[i-2]) aux_dir=0;
              if (aux_dir<-2 && VWAP_CROSS && force_trade==0 && low[i-2]>VWAP[i-2] && CCI[i-1]<-150 && close[i-1]<loBand[i-1] && high[i-2]>upBand[i-2] && high[i-3]>upBand[i-3] && close[i-1]<MAFast[i-1]) aux_dir=0;
              if (force_trade==0 && filter.tdf_color==0) {
                 if (aux_dir<-2 && filter.VWAP_CROSS && pricestats.bars_day<6 && low[i-1]<MA200[i-1] && close[i-1]>MA200[i-1]) aux_dir=0;
                 if (aux_dir<-2 && filter.VWAP_LASTCROSS && filter.HILO_INVERTSELL && CCI[i-1]<-60 && candle_lo>0 && open[i-1]<MAFast[i-1] && low[i-3]>VWAP[i-3] && aux_abs<aux_abslast/2 && MathAbs(close[i-3]-open[i-3])<aux_abslast/2) aux_dir=0;
                 if (aux_dir<-2 && filter.NO_VWAPCROSS && filter.VWAP_UP && i-pricestats.vwap_idx>30 && CCI[i-1]<-150 && low[i-1]>VWAP[i-1] && low[i-1]>MA50[i-1] && ((low[i-1]<MA50[i-1] && close[i-1]>MA50[i-1]) || (low[i-1]<MA100[i-1] && close[i-1]>MA100[i-1]))) aux_dir+=2;
                 if (aux_dir<-2 && filter.NO_VWAPCROSS && filter.VWAP_UP && filter.MIBAND_UP && CCI[i-1]<-200 && candle_lo>0 && stdDev[i-1]<aux_abs/2 && i-pricestats.vwap_idx>20 && low[i-1]>VWAP[i-1] && MA50[i-1]>VWAP[i-1] && MA50[i-1]-MA100[i-1]>aux_abs && MA25[i-1]-MA50[i-1]>aux_abs/2 && MA100[i-1]>MA100[i-2]) aux_dir=0;
                 if (aux_dir<-2 && filter.NO_VWAPCROSS && filter.HILO_INVERTSELL && date_candle.hour==13 && CCI[i-1]<-100 && MFI[i-1]<15 && candle_lo>0 && low[i-1]>VWAP[i-1] && low[i-1]-VWAP[i-1]<body_down/2) aux_dir=0;
                 if (aux_dir<-2 && filter.NO_VWAPCROSS && filter.HILO_INVERTSELL && date_candle.hour==13 && mycounters.c_down==1 && i-pricestats.vwap_idx>30 && CCI[i-1]<-170 && candle_lo>0 && high[i-1]<VWAP[i-1]) aux_dir=0;
                 if (aux_dir<-2 && filter.NO_VWAPCROSS && filter.VWAP_UP && filter.HILO_BUY && MA25[i-1]>VWAP[i-1] && close[i-1]>VWAP[i-1] && low[i-1]<HILO[i-1] && close[i-1]>HILO[i-1] && low[i-1]<MA25[i-1] && close[i-1]>MA25[i-1]) aux_dir++;
              }
           }
        }
     }
     ...
     if (date_candle.hour>10 && date_candle.hour<15) {
        if (type==0) {
           ...
           if (apftrade.trade_dir==1) {
              ...
              if (CCI[i-2]>120 && RSI[i-2]>70 && body_down>3*candle_lo && close[i-1]<low[i-2] && ((close[i-3]<MA100[i-3] && close[i-2]>MA100[i-2] && close[i-1]>VWAP[i-1]) || (close[i-3]<MA200[i-3] && close[i-2]>MA200[i-2])) && close[i-1]<MA50[i-1] && high[i-2]>upBand[i-2] && close[i-1]<upBand[i-1]) aux_dir=-5;
           }
              ...
              if (CCI[i-2]<-120 && RSI[i-2]<30 && body_up>3*candle_hi && close[i-1]>high[i-2] && ((close[i-3]>MA100[i-3] && close[i-2]<MA100[i-2] && close[i-1]<VWAP[i-1]) || (close[i-3]>MA200[i-3] && close[i-2]<MA200[i-2])) && close[i-1]>MA50[i-1] && low[i-2]<loBand[i-2] && close[i-1]>loBand[i-1]) aux_dir=5;
           }
        }
