     ...
     if (force_trade==0) {
        ...
        if (type==1) {
           if (aux_dir>2) {
              ...
              if (aux_dir>2 && aux_dir<14 && filter.tdf_color==0 && mycounters.hilobuy>5 && CCI[i-1]>70 && CCI[i-1]-CCI[i-2]<1 && close[i-1]<high[i-2] && candle_hi>0 && low[i-1]>VWAP[i-1] && (tick_volume[i-1]<tick_volume[i-2] || tick_volume[i-1]<tick_volume[i-3])) aux_dir-=2;
