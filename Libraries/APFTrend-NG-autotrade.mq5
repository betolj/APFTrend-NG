     ...
     if (CCI[i-1]>50 && (type==0 || (type==1 && VWAP[i-1]>0 && date_candle.hour<17))) {
        ...
        if (aux_dir>1 && (type==1 || apftrade.trade_dir==1)) {
           ...
           if (type==1 && aux_dir>2) {
              ...
              //if (aux_dir>2 && filter.tdf_color!=1 && force_trade==0 && (date_candle.hour<10 || (date_candle.hour==10 && date_candle.min<15)) && high[i-1]>VWAP[i-1] && low[i-1]<VWAP[i-1] && high[i-1]>MA200[i-1] && low[i-1]<MA200[i-1]) aux_dir=0;
        ...
        if (aux_dir<-1 && (type==1 || apftrade.trade_dir==2)) {
           ...
           if (type==1 && aux_dir<-2) {
              ...
              if (aux_dir<-2 && filter.tdf_color!=2 && force_trade==0 && (date_candle.hour<10 || (date_candle.hour==10 && date_candle.min<15)) && high[i-1]>VWAP[i-1] && low[i-1]<VWAP[i-1] && high[i-1]>MA200[i-1] && low[i-1]<MA200[i-1]) aux_dir=0;
     ...
     if (force_trade==0) {
        if (filter.VWAP_CROSS && filter.VWAP_LASTCROSS) {
           if (aux_dir>2 && mycounters.c_up>1 && candle_hi>0) {
              if (close[i-2]>MAFast[i-2] && high[i-1]<MA200[i-1] && MA200[i-1]<MA200[i-2] && close[i-1]>VWAP[i-1] && (close[i-1]<high[i-2] || close[i-1]<high[i-3])) aux_dir=0;
              else if (date_candle.hour>12 && filter.tdf_color==0 && RSI[i-1]>65 && MFI[i-1]>70 && filter.VWAP_CROSS && filter.VWAP_LASTCROSS && mycounters.c_up>1 && body_up<aux_abslast) aux_dir=0;
           }
           else if (aux_dir<-2 && mycounters.c_down>1 && candle_lo>0) {
              if (close[i-2]<MAFast[i-2] && low[i-1]>MA200[i-1] && MA200[i-1]>MA200[i-2] && close[i-1]<VWAP[i-1] && (close[i-1]>low[i-2] || close[i-1]>low[i-3])) aux_dir=0;
              else if (date_candle.hour>12 && filter.tdf_color==0 && MFI[i-1]<30 && filter.VWAP_CROSS && filter.VWAP_LASTCROSS && mycounters.c_down>1 && body_down<aux_abslast) aux_dir=0;
           }
        }
        if (type==1) {
           if (aux_dir>2) {
